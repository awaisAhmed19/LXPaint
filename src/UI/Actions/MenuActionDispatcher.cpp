...
void doUndo(Editor& editor) {
    // CommandManager::undo is not exposed through Editor yet;
    // add Editor::undo() / Editor::redo() accessors when wiring this up.
    // Logged as a reminder rather than silently no-oping.
    Logger::warn("MenuAction::EditUndo — Editor::undo() not yet exposed");
    (void)editor;
}

void doRedo(Editor& editor) {
    Logger::warn("MenuAction::EditRedo — Editor::redo() not yet exposed");
    (void)editor;
}
