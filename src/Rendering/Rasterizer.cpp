...
void drawVerticalSpan(SDL_Surface* surface, int x, int y, int thickness, uint32_t color,
                      bool useXOR) {
...
void drawHorizontalSpan(SDL_Surface* surface, int x, int y, int thickness, uint32_t color,
                        bool useXOR) {
...
void drawEllipse(SDL_Surface* surface, int xc, int yc, int rx, int ry, uint32_t color) {
    float dx, dy, d1, d2;
    int x = 0;
    int y = ry;
    ...
    // Region 2
    d2 = ((ry * ry) * ((x + 0.5f) * (x + 0.5f))) + ((rx * rx) * ((y - 1) * (y - 1))) -
         (rx * rx * ry * ry);
...
void drawCircle(SDL_Surface* surface, int x_centre, int y_centre, int r, uint32_t color) {
    int x = r;
    int y = 0;
...
void rectFill(SDL_Surface* surface, int minX, int minY, int maxX, int maxY, uint32_t color) {
...
void rectFillWhite(SDL_Surface* surface, int minX, int minY, int maxX, int maxY) {
    if (!lockSurface(surface)) return;
...
void drawRect(SDL_Surface* surface, vec2 a, vec2 b, uint32_t color, int brushSize) {
    ...
    bresenham({(float)minX, (float)minY}, {(float)maxX, (float)minY}, surface, color, brushSize,
              false);

    bresenham({(float)minX, (float)maxY}, {(float)maxX, (float)maxY}, surface, color, brushSize,
              false);

    bresenham({(float)minX, (float)minY}, {(float)minX, (float)maxY}, surface, color, brushSize,
              false);

    bresenham({(float)maxX, (float)minY}, {(float)maxX, (float)maxY}, surface, color, brushSize,
              false);

    rectFill(surface, minX, minY, maxX, maxY, color);
}

void dda(vec2 start, vec2 end, SDL_Surface* surface, uint32_t color, int brushSize, bool useXOR) {
...
void bresenham(vec2 start, vec2 end, SDL_Surface* surface, uint32_t color, int brushSize,
               bool useXOR) {
