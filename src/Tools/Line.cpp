...
void Line::onMouseMove(vec2<float> pos, Canvas& canvas) {
    if (!drawing || !currentSnapshot) return;
    ...
    // --- PHASE 3: LOGGING ---
    float tBres = std::chrono::duration_cast<std::chrono::microseconds>(e1 - s1).count();
    float tDDA = std::chrono::duration_cast<std::chrono::microseconds>(e2 - s2).count();

    Profiler::recordRaceStep(
        {{"BRESENHAM", tBres, ImVec4(1, 0.5f, 0, 1)}, {"DDA", tDDA, ImVec4(0, 0.5f, 1, 1)}}, pos);
...
Command* Line::onMouseUp(vec2<float> pos, Canvas& canvas) {
    ...
    Profiler::commitRace({{"BRESENHAM", ImVec4(1, 0.5f, 0, 1)}, {"DDA", ImVec4(0, 0.5f, 1, 1)}});
    return cmd;
}

void Line::bresenham(vec2<float> start, vec2<float> end, Canvas& canvas, uint32_t color,
                     int brushSize, bool useXOR) {
    if (!lockSurface(canvas.drawingSurface)) return;
...
void Line::dda(vec2<float> start, vec2<float> end, Canvas& canvas, uint32_t color, int brushSize,
               bool useXOR) {
