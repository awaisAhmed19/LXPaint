...
void Line::onMouseMove(vec2<float> pos, Canvas &canvas) {
  ...
  case OptimizationMode::OVERLAY:
    ...
    Renderer::bresenham(startpos, lastMousePos, canvas, color, brushSize);
    break;
  }
  ...
  Renderer::bresenham(startpos, pos, canvas, color, brushSize);
  auto e1 = std::chrono::high_resolution_clock::now();

  auto s2 = std::chrono::high_resolution_clock::now();
  Renderer::dda(startpos, pos, canvas, color, brushSize);
