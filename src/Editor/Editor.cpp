...
Editor::Editor(SDL_Renderer* renderer)
    : m_canvas(800, 550), m_preview(800, 550), m_renderer(renderer), m_commands(50, 256) {
    setupTools();
    setupInputBindings();
...
void Editor::setupInputBindings() {
    ...
    m_input.bindActions(InputCommand::PENCIL, [this]() { m_tools.setActiveTool(ToolID::Pencil); });
    m_input.bindActions(InputCommand::CIRCLE, [this]() { m_tools.setActiveTool(ToolID::Circle); });
    m_input.bindActions(InputCommand::LINE, [this]() { m_tools.setActiveTool(ToolID::Line); });
    m_input.bindActions(InputCommand::RECT, [this]() { m_tools.setActiveTool(ToolID::Rect); });
    m_input.bindActions(InputCommand::ERASER, [this]() { m_tools.setActiveTool(ToolID::Eraser); });
}

ToolContext Editor::makeToolContext() {
    return ToolContext{.canvas = &m_canvas, .preview = &m_preview, .interaction = &m_interaction};
}

bool Editor::inCanvas(vec2 mousePos) {
    return mousePos.x >= 0 && mousePos.y >= 0 && mousePos.x < m_canvas.getWidth() &&
           mousePos.y < m_canvas.getHeight();
}
...
void Editor::handleEvent(const SDL_Event& e) {
    ...
    */
    ...
    if (m_input.leftMousePressed()) {
        ...
        vec2 mousePos = m_viewport.screenToCanvas(m_input.getMouseScreenPos(), m_docTransform);

        if (!inCanvas(mousePos)) return;
    ...
    */
    ...
    if (m_input.leftMouseReleased()) {
        if (!m_interaction.active) return;
